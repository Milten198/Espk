...
public class BaseRatingBar extends LinearLayout implements SimpleRatingBar {
    ...
    private void verifyParamsValue() {
        ...
        if (mEmptyDrawable == null) {
            mEmptyDrawable = VectorDrawableCompat.create(getContext().getResources(), R.drawable.ic_star_empty_12dp, null);
        }

        if (mFilledDrawable == null) {
            mFilledDrawable = VectorDrawableCompat.create(getContext().getResources(), R.drawable.ic_star_white_12dp, null);
        }
    ...
    public void setRating(float rating) {
        ...
        if (rating <= 0.0f && mPreviousRating > 0.0f) {
            rating = mPreviousRating;
        }
